...
Internal generation may include a first draft, multiple candidates, and multiple targeted repair passes. This does not change the delivery count. Confirm that the single selected final image uses the user's confirmed ratio; it is unmistakably the same person; the requested emotion is immediately readable or the default expression is naturally happy, lively, and approachable; its eyes and facial landmarks are crisp while forehead, temples, both cheeks, under-eyes, nose skin, perioral area, and chin are spotless and silky smooth with continuous gradients and no isolated red/brown/gray dot, acne residue, freckle, pigmentation, surface scar, recognizable pore, facial grain, orange-peel texture, or sharpening grit; its complexion is brighter, translucent, rosy, and hydrated; both under-eye zones transition cleanly into the cheeks with no tear trough, eye bag, tear track, brown-orange band, dark crease, or rough texture; makeup is clearly visible; anatomy and wardrobe are coherent; the person is integrated into one detailed black-gold space; and all double exposure, bloom, haze, and glow stay outside the central face. One residual blemish fails acceptance. If any check fails, keep working privately through another candidate or targeted identity-preserving edit while progress is being made. Compare passing candidates and choose the highest-quality one. Never present a draft, rejected image, repair pass, source/result pair, or alternate. After all internal work, display or attach the selected final exactly once so the response contains one image card only.
