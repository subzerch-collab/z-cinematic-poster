...
7. visibly bright, milky-translucent, spotless, silky-smooth, hydrated skin with a soft rosy undertone, continuous tonal gradients, and controlled dewy sheen; fully remove every isolated red, brown, or gray dot, visible pore, grain, orange-peel texture, rough microcontrast, tear track, tear trough, eye bag, orange-brown under-eye band, dark crease, spot, freckle, acne and blemish mark, surface scar, dull patch, and under-eye darkness without leaving faint residue, while preserving facial geometry and identity;
...
Internal generation may include a first draft, any number of candidates or variations, batch generation, comparisons, and multiple targeted repair passes. Internal count and strategy are unrestricted by the one-image delivery rule. Confirm that the single selected final image uses the user's confirmed ratio; it is unmistakably the same person; the requested emotion is immediately readable or the default expression is naturally happy, lively, and approachable; its eyes and facial landmarks are crisp while forehead, temples, both cheeks, under-eyes, nose skin, perioral area, and chin are spotless and silky smooth with continuous gradients and no isolated red/brown/gray dot, acne residue, freckle, pigmentation, surface scar, recognizable pore, facial grain, orange-peel texture, or sharpening grit; its complexion is brighter, translucent, rosy, and hydrated; both under-eye zones transition cleanly into the cheeks with no tear trough, eye bag, tear track, brown-orange band, dark crease, or rough texture; makeup is clearly visible; anatomy and wardrobe are coherent; the person is integrated into one detailed black-gold space; and all double exposure, bloom, haze, and glow stay outside the central face. One residual blemish fails acceptance. If any check fails, keep working privately through further candidates or targeted identity-preserving edits while progress is being made. Compare all passing candidates and choose the highest-quality one. Never present a draft, rejected image, repair pass, source/result pair, alternate, contact sheet, or complete multi-image tool response. Extract or materialize the chosen candidate as a standalone single-image object or file, then pass only that one image—not an array or candidate collection—to the display mechanism exactly once, so the response contains one image card and one image asset only.
