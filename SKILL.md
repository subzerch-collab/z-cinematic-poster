...
For the visual treatment and ratio-specific composition, read [references/black-gold-campaign.md](references/black-gold-campaign.md). Build a concise generation prompt from the user's scene plus the confirmed ratio and defaults above. Treat the first output as a private draft. Before returning, compare every viable candidate with the reference and inspect the entire face at close range, including forehead, temples, both cheeks, the strip from the lower lash line through the upper cheek, nose, perioral area, and chin. Check: unmistakably the same person; the requested emotion is immediately readable, or the default expression looks naturally happy, lively, and approachable rather than tired or blank; exact requested aspect ratio; full uncropped head; natural anatomy; crisp eyes and facial landmarks without sharpened skin texture; evenly readable facial illumination; bright milky-translucent skin with a soft rosy undertone, smooth hydration, and continuous poreless-looking gradients; absolutely no isolated red, brown, or gray blemish dot, acne residue, dark spot, freckle, pigmentation patch, visible grain, enlarged pore, orange-peel texture, tear trough, eye bag, orange-brown under-eye rim, dark crease, tear track, or surface scar; makeup obvious at first glance; unified subject/background lighting; and optical effects outside the central face. A single visible blemish residue fails the check. If identity, expression, anatomy, integration, background, or face finish fails, privately generate another candidate or preserve the accepted composition and perform a targeted identity-preserving edit: adjust only the failed regions; reconstruct each blemished patch from the clean surrounding complexion; even the skin tone and under-eye zone; preserve facial form, natural light modeling, and the eye/nose/lip shapes; and keep facial landmarks crisp. Do not use global blur to hide marks. Continue while each pass makes meaningful progress and resources permit; stop after two consecutive non-improving passes or when no safe edit can improve the result. If no candidate passes, report that no acceptable final was produced instead of displaying a failed image. Follow the final-only display protocol: select the best passing candidate and surface it once, using one display action after all internal work. A single side-view or severely obscured face may require inference for unseen features; keep the closest reliable scene-compatible angle or ask for an additional view instead of claiming an uncertain match.
