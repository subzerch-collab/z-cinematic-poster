...
- Treat identity as the only non-negotiable invariant from the source portrait. Preserve the same person's facial structure and recognition cues: face shape and proportions, eye shape and spacing, brows, nose, lips, jawline, apparent age, hairline, ethnicity, underlying complexion, and distinguishing features. Beauty-grade brightening and rosy color correction are allowed, but must not turn the person into someone else. Do not preserve the original pose, crop, gaze, head tilt, expression, clothing, or accessories merely because they appear in the reference.
...
- Apply a clearly visible high-end beauty-campaign finish by default, not a barely perceptible cleanup. Make the face bright, translucent, smooth, hydrated, and softly rosy, with an even luminous complexion and controlled dewy highlights on the forehead, cheeks, nose, and lips. Fully remove tear tracks, dark fine lines, under-eye creases and darkness, uneven pigmentation, spots, freckles, acne marks, blemish marks, visible surface scars, redness, roughness, dry texture, and mottled shadow. Add makeup that is immediately visible at first glance: refined base, shaped brows, clean eyeliner or eye definition, separated lashes, healthy blush, subtle contour and highlight, softly colored glossy lips, polished hair, and overall grooming. Keep only ultra-fine uniform skin texture; never retain coarse pores, dark grooves, crease-like texture, or patterned denoise artifacts. Preserve facial geometry, eye/nose/lip shape, apparent age, ethnicity, hairline, and the person's overall identity; surface marks are editable and should be cleaned unless the user asks to keep them. Beauty must come from skin finish, makeup, grooming, expression, and light—not from changing who the person is.
- Keep the primary facial landmarks optically sharp at the highest supported native resolution. The irises, catchlights, eyelashes, brows, nostril edges, lip line, and hair strands must have clean focus and precise edges, while facial skin stays smooth, continuous, and free of emphasized pore noise. Never sharpen skin texture or apply soft focus, motion blur, diffusion, bloom, haze, double exposure, denoise smearing, or glow across the central face; restrict those effects to the background, hair perimeter, outer silhouette, and secondary afterimage.
...
## Final-only display protocol

Treat every generated or edited image as private working state until all identity, skin, under-eye, makeup, sharpness, composition, and ratio checks pass.
...
- When the environment supports deferred presentation, keep generation and edit results internal. In Codex code-mode, do not call `generatedImage(result)` or any equivalent image-display helper for intermediate calls.
- After the last repair and validation step, invoke the image-display or attachment mechanism exactly once with only the newest accepted result. The response must contain one image card, not two visually similar cards.
- If an image tool would automatically surface every generation call, avoid visible multi-call retries: combine the required skin and under-eye corrections into the single final generation/edit request, then display only that result.
...
For the visual treatment and ratio-specific composition, read [references/black-gold-campaign.md](references/black-gold-campaign.md). Build a concise generation prompt from the user's scene plus the confirmed ratio and defaults above. Before returning, compare the generated face with the reference and inspect the face at close range, especially the strip from the lower lash line through the tear trough to the upper cheek. Check: unmistakably the same person; exact requested aspect ratio; full uncropped head; natural anatomy; crisp eyes and facial landmarks without sharpened skin texture; evenly readable facial illumination; bright milky-translucent skin with a soft rosy undertone and smooth hydration; no tear trough, eye bag, orange-brown under-eye rim, dark crease, tear track, spot, acne mark, or blemish mark; makeup obvious at first glance; unified subject/background lighting; and optical effects outside the central face. If the under-eye or skin check fails, keep the accepted poster composition and perform a localized identity-preserving face-retouch edit rather than regenerating the whole scene: brighten and color-correct the under-eye zone, blend it smoothly into the cheek, clean surface defects, preserve the eye shape, and keep the eyes and facial landmarks crisp. Allow at most two hidden localized repair attempts. Follow the final-only display protocol: surface only the newest accepted result after all repairs, using one display action. A single side-view or severely obscured face may require inference for unseen features; keep the closest reliable scene-compatible angle or ask for an additional view instead of claiming an uncertain match.
